(() => {
  ...
  fetch('/api/index.php?action=public-content').then(r => r.ok ? r.json() : Promise.reject()).then(data => {
    ...
    const legacySlug = document.body.dataset.creationSlug;
    const creation = legacySlug && data.creations.find(item => item.slug === legacySlug);
    if (creation) {
      document.querySelectorAll('[data-creation-title]').forEach(node => node.textContent = field(creation, 'title'));
      document.querySelectorAll('[data-creation-meta]').forEach(node => node.textContent = field(creation, 'meta'));
      document.querySelectorAll('[data-creation-intro]').forEach(node => node.textContent = field(creation, 'intro'));
      document.querySelectorAll('[data-creation-body]').forEach(node => node.textContent = field(creation, 'body'));
      document.querySelectorAll('[data-creation-hero]').forEach(node => { node.src = creation.hero; node.alt = field(creation, 'title'); });
      document.querySelectorAll('[data-creation-gallery]').forEach(node => node.innerHTML = creation.photos.map(url => `<img class="ph" src="${escape(url)}" alt="${escape(field(creation, 'title'))}">`).join(''));
    }
  }).catch(() => {});
})();
